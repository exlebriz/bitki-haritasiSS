...
# Tür referansını doğrudan aktif veriden üretir (Excel'den gelen ne varsa o).
# Aynı bilimsel ad için birden fazla Türkçe ad/kod varsa en sık geçeni seçilir.
build_species_ref <- function(d) {
  bos <- data.frame(Bilimsel = character(), Turkce = character(),
                    Kod = character(), stringsAsFactors = FALSE)
  if (is.null(d) || !nrow(d)) return(bos)

  ct <- data.frame(
    Bilimsel = enc2utf8(trimws(as.character(d$Bilimsel))),
    Turkce   = enc2utf8(trimws(as.character(d$Turkce))),
    Kod      = enc2utf8(trimws(as.character(d$Tur))),
    stringsAsFactors = FALSE)
  ct[is.na(ct)] <- ""
  ct$Kod[ct$Kod == "\u2014"] <- ""                      # "—" kodu boş sayılır
  ct <- ct[nzchar(ct$Bilimsel) & ct$Bilimsel != "Belirtilmemi\u015f", , drop = FALSE]
  if (!nrow(ct)) return(bos)

  en_sik <- function(x) { x <- x[nzchar(x)]
    if (!length(x)) "" else names(sort(table(x), decreasing = TRUE))[1] }

  ct <- do.call(rbind, lapply(split(ct, ct$Bilimsel), function(g)
    data.frame(Bilimsel = g$Bilimsel[1],
               Turkce   = en_sik(g$Turkce),
               Kod      = en_sik(g$Kod), stringsAsFactors = FALSE)))
  ct <- ct[order(ct$Bilimsel), , drop = FALSE]
  rownames(ct) <- NULL
  ct
}
...
parse_tarih <- function(x) {
  ...
  x[is.na(x)] <- ""                      # <<< HATA BURADAYDI: NA'ları boş metne çevir
  out <- rep(NA_character_, length(x))
  
  iso <- grepl("^[0-9]{4}-[0-9]{2}-[0-9]{2}", x);  out[iso] <- substr(x[iso], 1, 10)
  yr  <- grepl("^[0-9]{4}$", x) & is.na(out);      out[yr]  <- x[yr]
  
  # gg.aa.yyyy / gg/aa/yyyy  (verinizde 13.09.2026 ve 11.10.2026 gibi 7 kayıt var)
  dmy <- grepl("^[0-9]{1,2}[./-][0-9]{1,2}[./-][0-9]{4}$", x) & is.na(out)
  if (any(dmy)) {
    d <- as.Date(gsub("[/-]", ".", x[dmy]), format = "%d.%m.%Y")
    out[dmy] <- ifelse(is.na(d), x[dmy], as.character(d))
  }
  
  ser <- grepl("^[0-9]{5}(\\.[0-9]+)?$", x) & is.na(out)
  out[ser] <- as.character(as.Date(round(as.numeric(x[ser])), origin = "1899-12-30"))
  
  rest <- is.na(out) & nzchar(x)
  out[rest] <- x[rest]
  out
}
...
parse_plant_data <- function(df) {
  ...
  # --- Boş satır/sütun ve başlıksız sütun temizliği ---
  bos_sut <- vapply(df, function(k) all(is.na(k) | trimws(as.character(k)) == ""), logical(1))
  adsiz   <- grepl("^\\.{3}[0-9]+$|^Unnamed", names(df))      # ...9, ...10, Unnamed: 8
  df <- df[, !(bos_sut | adsiz), drop = FALSE]
  bos_sat <- apply(df, 1, function(r) all(is.na(r) | trimws(as.character(r)) == ""))
  df <- df[!bos_sat, , drop = FALSE]
  if (!nrow(df)) stop("Veri bo\u015f g\u00f6r\u00fcn\u00fcyor.")
  nm <- names(df)
  ...
  bos_id <- is.na(idv) | !nzchar(trimws(idv)) | idv == "NA"
  if (any(bos_id)) {
    mx <- suppressWarnings(max(as.integer(gsub("[^0-9]", "", idv[!bos_id])), na.rm = TRUE))
    if (!is.finite(mx)) mx <- 0
    idv[bos_id] <- as.character(mx + seq_len(sum(bos_id)))
  }
  out <- data.frame(
    ID       = idv,
...
load_default <- function() {
  bos <- data.frame(ID = character(), ID2 = character(), Tarih = character(), Tur = character(),
                    Bilimsel = character(), Turkce = character(),
                    Enlem = numeric(), Boylam = numeric(), stringsAsFactors = FALSE)
  if (!file.exists(DEFAULT_DATA_PATH)) {
    message("UYARI: ", normalizePath(DEFAULT_DATA_PATH, mustWork = FALSE), " bulunamadi.")
    return(bos)
  }
  raw <- tryCatch(readxl::read_excel(DEFAULT_DATA_PATH, sheet = 1, col_types = "text",
                                     .name_repair = "unique_quiet"),
                  error = function(e) { message("Excel okunamadi: ", conditionMessage(e)); NULL })
  if (is.null(raw)) return(bos)
  res <- tryCatch(parse_plant_data(as.data.frame(raw)),
                  error = function(e) { message("Ayristirma hatasi: ", conditionMessage(e)); NULL })
  if (is.null(res)) bos else res
}
# ---------------------------------------------------------------------
# 1. Tema
# ---------------------------------------------------------------------
...
server <- function(input, output, session) {
  ...
  acik_ekle_modal <- function() {
    d <- current_data()
        ct  <- combo_table()
    sp  <- ct$Bilimsel
    kod <- sort(unique(ct$Kod[nzchar(ct$Kod)]))
    trk <- sort(unique(ct$Turkce[nzchar(ct$Turkce)]))
    p <- isolate(rv$pt)
    if (is.null(p)) {
    ...
    showModal(modalDialog(
      ...
      options = list(create = TRUE, createOnBlur = TRUE, maxOptions = 5000,
                     placeholder = "\u00f6rn. Cistus creticus")),
      fluidRow(
        column(6, selectizeInput("m_tur", "T\u00fcr (kod)", choices = kod, selected = character(0),
          options = list(create = TRUE,  maxOptions = 5000, placeholder = "\u00f6rn. CCR"))),
        column(6, selectizeInput("m_turkce", "T\u00fcrk\u00e7e ad", choices = trk,
          selected = character(0),
          options = list(create = TRUE,  maxOptions = 5000, placeholder = "\u00f6rn. Pembe laden")))),
  ...
  observeEvent(input$ekle_harita,   acik_ekle_modal())
   ...
   # Referans tablo artık %100 aktif veriden (yüklenen Excel'den) geliyor
  combo_table <- reactive({ build_species_ref(current_data()) })
